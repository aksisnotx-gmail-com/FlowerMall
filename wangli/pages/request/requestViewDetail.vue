	...
	<view class="container">
		<view class="container">
			...
			<view class="c-list" :style="{backgroundColor:request.backgroundColor,background:'url('+request.background+')'}">
				...
				<view class="seckill-section m-t" v-if="responseData.length != 0">
					<scroll-view class="floor-list" scroll-x>
						<view class="scoll-wrapper">
							<view 
								v-for="(responseItem, responseIndex) in responseData" :key="responseIndex"
								class="floor-item"
								@click="viewResponse(responseItem.id)"
							>
								<image :src="responseItem.tagUrl" mode="aspectFill"></image>
								<text class="title clamp">{{responseItem.name}}</text>
							</view>
						</view>
					</scroll-view>
				</view>
				<view v-if="this.responseData.length == 0">
					<text>暂无回复信息</text>
				</view>
			</view>
		</view>
...
<script>
	import {getRequestInfo,getResponseInfo} from "@/api/request/requestViewDetail.js"
	...
	export default{
		...
		methods:{
			viewResponse(id){
				uni.navigateTo({
					url: `/pages/response/responseViewDetail_user?id=${id}&requestId=${this.request.id}`
				})
			},
			goBack(){
				uni.navigateBack();
