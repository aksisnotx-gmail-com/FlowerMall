<template>
	<view class="container">
				...
				<view class="top-title-info">
					<view class="top-title-list" :style="{backgroundColor:backgroundColor}">
						<view class="top-title-row b-b">
							...
							<text class="top-title-text" :style="{color:contentColor}">{{product.name}}</text>
						</view>
						<view class="top-title-row b-b">
							<text class="top-title-tit" :style="{color:tagColor}">区域：</text>
							<text class="top-title-text" :style="{color:contentColor}">{{product.areaTxt}}</text>
						</view>
						<view class="top-title-row b-b">
							<text class="top-title-tit" :style="{color:tagColor}">类型：</text>
							<text class="top-title-text" :style="{color:contentColor}">{{product.typeTxt}}</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="c-list" :style="{backgroundColor:backgroundColor,background:'url('+background+')'}">
			...
			<view class="c-row b-b">
				...
				<view class="con" :style="{backgroundColor:backgroundColor}">
					<text type="textarea" :style="{color:contentColor}">{{product.description}}</text>
				</view>
			</view>
			<view class="c-row b-b">
				<text class="tit" :style="{color:tagColor}">讲价：</text>
				<view class="con">
					<text  class="tit desc-text":style="{color:contentColor}">{{bargainText}}</text>
				</view>
			</view>
			<view class="c-row b-b">
				<text class="tit" :style="{color:tagColor}">自动上架：</text>
				<view class="con">
					<text class="tit desc-text" :style="{color:contentColor}">{{publishText}}</text>
				</view>
			</view>
			<view class="introduce-section">
				<text>其他</text>
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group">
				<button type="primary" class=" action-btn no-border add-cart-btn" @click="submitProduct">保存</button>
			</view>
		</view>
	</view>
</template>
	...
	export default{
		...
		async onLoad(options){
			...
			if(this.tagImg == undefined){
				...
				this.product.tagUrl = this.tagImg.url;
			}
			this.bargainText = prePage.$vm.bargainText;
			this.publishText = prePage.$vm.publishText;
			console.log("this.bargainFlagTxt"+this.bargainText)
			console.log("this.publishTxt"+this.publishText)
		},
		methods:{
			async submitProduct(){
				uni.showLoading({
				...
				if(this.backgroundImg != "" && this.backgroundImg != undefined){
					...
					await uploadTag(background).then(response => {
						this.product.background = response.imgUrl
					});
				}
				...
				this.product.backgroundColor = this.backgroundColor;
				this.product.tagColor = this.tagColor;
				this.product.contentColor = this.contentColor;
				
				let addData = {'product':this.product,'formData':this.formData};
				saveProduct(addData).then(response => {
					this.$modal.msgSuccess("商品创建成功")
					setTimeout(function() {
						uni.switchTab({
