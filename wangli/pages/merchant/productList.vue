<template>
	<view class="container">
					...
					>
						...
						<view class="item-right c-list">
							...
							<view class="c-row b-b">
								...
								<view class="con">
									<switch :checked="item.publish" @change="switchChange(item.id)"></switch>
								</view>
							</view>
							<view class="c-row b-b">
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group">
				<button type="primary" class=" action-btn no-border buy-now-btn" @click="productConfirm">确定</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	import {getProductInfoList,deleteProductInfo,updateProduct} from "@/api/merchant/productList.js"
	export default {
		data() {
			return {
				total: 0, //总价格
			...
			productConfirm(){
				...
				for(let data of this.cartList){
					if(data.checked){
						...
						selectProduct.push(data.id)
					}
				}
				let updateData = {'merchantId':this.merchantId,'productId':selectProduct.join()}
