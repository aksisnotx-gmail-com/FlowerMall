					...
					>
						<view class="image-wrapper">
							<image :src="item.tagUrl" 
								...
								mode="aspectFill" 
								lazy-load 
								@click="toMerchantDetail(item.id,item.status)"
						...
						<view class="item-right c-list">
							...
							<view class="c-row b-b">
								<view class="con" v-if="item.status == '3'">
									<uni-icons type="weixin" size="30" color="#a6e7f1" @click="shareMerchant(item)"></uni-icons>
								</view>
								<view class="con">
									<uni-icons type="trash" size="30" color="#a6e7f1" @click="deleteMerchant(item.id)"></uni-icons>
								</view>
...
<script>
	...
	const merchantShareWebPath = config.merchantShareWebPath
	
	import {getMerchantList,deleteMerchantInfo,updatePublish,initMerchantSign} from "@/api/merchant/merchantList.js"
	...
	export default {
		...
		methods: {
			shareMerchant(item){
				...
				initMerchantSign(item.id).then(response => {
					let ticket = response.data.ticket;
					let webUrl = uniUrl + merchantShareWebPath+'?ticket='+ticket;
			...
			deleteMerchant(id){
				deleteMerchantInfo(id).then(response => {
					this.$api.msg(response.msg);
					this.loadData();
				});
			},
			toMerchantDetail(id,status){
				uni.navigateTo({
					url: `/pages/merchant/merchantView?id=${id}`
				})
			}
		}
	}
