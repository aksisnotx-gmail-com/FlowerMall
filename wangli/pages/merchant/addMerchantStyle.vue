	...
	<view class="container">
				...
				<view class="top-title-info">
					<view class="top-title-list" :style="{backgroundColor:backgroundColor}">
						<view class="top-title-row b-b">
							...
							<text class="top-title-text" :style="{color:contentColor}">{{merchant.name}}</text>
						</view>
						<view class="top-title-row b-b">
							<text class="top-title-tit" :style="{color:tagColor}">区域：</text>
							<text class="top-title-text" :style="{color:contentColor}">{{merchant.areaText}}</text>
						</view>
						<view class="top-title-row b-b">
							<text class="top-title-tit" :style="{color:tagColor}">联系人：</text>
							<text class="top-title-text" :style="{color:contentColor}">{{merchant.userName}}</text>
						</view>
						<view class="top-title-row b-b">
							<text class="top-title-tit" :style="{color:tagColor}">联系电话：</text>
							<text class="top-title-text" :style="{color:contentColor}">{{merchant.phone}}</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="c-list" :style="{backgroundColor:backgroundColor,background:'url('+background+')'}">
			...
			<view class="c-row b-b">
				<text class="tit" :style="{color:tagColor}">简介</text>
				<view class="con" :style="{backgroundColor:backgroundColor}">
					<text type="textarea" :style="{color:contentColor}">{{merchant.description}}</text>
				</view>
			</view>
			<view class="c-row b-b">
				<text class="tit" :style="{color:tagColor}">详细地址</text>
				<view class="con" :style="{backgroundColor:backgroundColor}">
					<text class="title text-center" :style="{color:contentColor}">{{merchant.address}}</text>
				</view>
			</view>
			<view class="c-row b-b">
				<text :style="{color:tagColor}">材料(证明你是你，能让用户相信你的材料)</text>
			</view>
			<view class="c-row b-b">
				<view class="con">
			...
			<view class="c-row b-b">
				<text class="tit" :style="{color:tagColor}">标签</text>
				<view class="con">
			...
			<!-- <uni-forms-item label="标签" required :style="{color:tagColor}">
				<uni-data-checkbox v-model="selectTag" multiple :localdata="this.tag" mode="tag" selected-color="#a6e7f1" selected-text-color="#ff00ff" max="3"/>
			</uni-forms-item> -->
			<view class="introduce-section">
				<text>其他</text>
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group">
				<button type="primary" class=" action-btn no-border buy-now-btn" @click="submitMerchant">保存</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	import {uploadTag,uploadFile,saveMerchant} from "@/api/merchant/addMerchant.js"
	export default{
		data() {
			return {
				merchant: {
		...
		onLoad(){
			...
			this.file = prePage.$vm.$refs.refFile.getFiles();
			this.merchant = prePage.$vm.merchant;
			this.formData = prePage.$vm.formData;
			this.tagImg = prePage.$vm.tagImg;
			if(this.tagImg == undefined){
				this.merchant.tagUrl = "";
			}else{
				this.merchant.tagUrl = this.tagImg.url;
			}
			this.txt = prePage.$vm.txt;
			let tags = prePage.$vm.tag;
			// let tags = prePage.$vm.selectTag;
			this.selectTag = prePage.$vm.selectTag;
			for(let tag of tags){
				if(this.selectTag.includes(tag.value) ){
					Object.defineProperty(tag,"disable",{
						value: true,
						writable : true,
						enumerable : true,
						configurable : true
		...
		methods:{
			async submitMerchant(){
				...
				this.merchant.tag = this.selectTag.join();
				...
				if(this.tagImg != "" && this.tagImg != undefined){
					...
					await uploadTag(tag).then(response => {
						this.merchant.tagUrl = response.imgUrl
					});
				}
				...
				if(this.backgroundImg != "" && this.backgroundImg != undefined){
					...
					await uploadTag(background).then(response => {
						this.merchant.background = response.imgUrl
					});
				}
				...
				let fileImgs = this.file;
				if(fileImgs.length > 0){
					let files = [];
					for(let fileImg of fileImgs){
						let data = {name: "file", filePath: fileImg.url,extname:fileImg.extname};
						await uploadFile(data).then(response => {
							files.push(response.imgUrl);
						});
					}
					this.merchant.url = files.join();
				}
				...
				this.merchant.backgroundColor = this.backgroundColor;
				this.merchant.tagColor = this.tagColor;
				this.merchant.contentColor = this.contentColor;
				
				let addData = {'merchant':this.merchant,'formData':this.formData};
				uni.hideLoading();
				saveMerchant(addData).then(response => {
					this.$modal.msgSuccess("商户创建成功")
					setTimeout(function() {
						uni.switchTab({
