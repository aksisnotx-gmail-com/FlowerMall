					...
					>
						...
						<view class="item-right c-list">
							...
							<view class="c-row b-b">
								...
								<text class="con">{{item.areaTxt}}</text>
							</view>
							<view class="c-row b-b">
								<view class="tit">类型：</view>
								<text class="con">{{item.typeTxt}}</text>
							</view>
							<view class="c-row b-b">
							...
							<view class="c-row b-b">
								...
								<text class="con">{{item.startTime}}----{{item.endTime}}</text>
							</view>
						</view>
					</view>
				</block>
...
<script>
	...
	import {getRequestList} from "@/api/sign/sign.js"
	export default {
		data() {
			return {
		...
		methods: {
			...
			async loadData(){
				this.cartList = [];
				await getRequestList().then(response => {
			...
			toRequestDetail(id){
				uni.navigateTo({
					url: `/pages/shareInfo/shareRequestViewDetail?id=${id}`
				})
			}
		}
	}
