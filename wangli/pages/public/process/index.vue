...
<script>
	...
	import {getProcessInfoList} from '@/api/public/process/index.js'
	export default {
		components:{
			pickerAddress
		},
		...
		onReachBottom(){
			...
			getProcessInfoList(data).then(response => {
		...
		methods: {
			...
			navToProcess(id) {
				//测试数据没有写id，用title代替
				uni.navigateTo({
					url: `/pages/public/process/processView?id=${id}`
				})
			},
			change(data) {
				let dataTmp = [];
