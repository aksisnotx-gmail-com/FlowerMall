<template>
	<view class="container">
		<uni-steps :options="detail.formData" :active="active" active-color="#a6e7f1"/>
		<view class="c-list">
		...
		<view class="content" >
			<view class="c-list" v-if="detail.formData.length > 0">
				<view v-for="(item,index) in detail.formData[active].item" :key="index">
				...
				<view class="introduce-section" v-if="detail.formData[active].requestList.length > 0">
					<text class="con">需求列表</text>
				</view>
				
				<view class="c-row">
					<view v-for="(item,index) in detail.formData[active].requestList" :key="index">
...
<script>
	...
	export default{
		...
		methods:{
			viewRequest(id){
				uni.navigateTo({
					url: `/pages/public/request/requestInfoDetail?id=${id}`
				})
			},
			nextStep() {
