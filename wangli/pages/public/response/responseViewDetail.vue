<template>
	<view class="container">
				...
				<view class="top-title-info">
					<view class="top-title-list">
						...
						<view class="top-title-row b-b">
							...
							<text class="top-title-text">{{response.userName}}</text>
						</view>
						<view class="top-title-row b-b">
							<text class="top-title-tit">创建时间：</text>
							<text class="top-title-text">{{formatDate(response.createTime)}}</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group" v-if="!hasAgree" @click="agreeResponse">
				<uni-icons type="hand-up-filled primary" class=" action-btn no-border add-cart-btn" color="#ffffff" size="30"></uni-icons>
			</view>
			<view class="action-btn-group" v-if="hasAgree">
				<uni-icons type="hand-up-filled primary" class=" action-btn no-border add-cart-btn" color="#ff0000" size="30"></uni-icons>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	import {addAgree,getResponseInfo,hasAgree,getUserInfo} from "@/api/sign/sign.js"
	...
	export default{
		...
		onLoad(options){
			getResponseInfo(options.id).then(response => {
		...
		methods:{
			validateStatus(){
				getUserInfo().then(response => {
					...
					if(this.haslogin && this.response.id != undefined){
						hasAgree(this.response.id).then(response => {
							if(response.data == '0'){
								this.hasAgree = false;
							}else{
								this.hasAgree = true;
			...
			goLogin(){
				let that = this;
				showConfirm('为了您的权益，支持此回复前，请先登录！').then(res => {
					if (res.confirm) {
						uni.navigateTo({
			...
			agreeResponse(){
				if(!this.haslogin){
					...
					showConfirm('每个需求，最多对五个回复进行点赞！').then(res => {
						if (res.confirm) {
							addAgree(this.response.id).then(response => {
								this.hasAgree = true;
							})
						}
					})
