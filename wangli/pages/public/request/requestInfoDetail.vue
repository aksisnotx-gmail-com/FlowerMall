	...
	<view class="container">
		...
		<view v-if="type == '1'">
			<view v-if="responseData.length > 0">
				<view class="scoll-item-title" v-for="(responseItem, responseIndex) in responseData" :key="responseItem.id" :style="{backgroundColor:request.backgroundColor,background:'url('+request.background+')'}">
		...
		<view v-if="type == '2'">
			<view v-if="message.length > 0">
				<view v-if="type == '2'" style="height: 100%; max-height: calc(100% - 100upx);">
		...
		<view class="page-bottom">
			<view class="action-btn-group" v-if="type != '2'">
				<button type="primary" class=" action-btn no-border" @click="goBack">返回</button>
			</view>
			<view class="action-btn-group" v-if="showResponse && type != '2'">
				<button type="primary" class=" action-btn no-border" @click="doResponse(request.id)">回复</button>
			</view>
			<view class="action-btn-group" v-if="type != '2'">
...
<script>
	...
	import {getRequestInfo,getResponseInfo} from "@/api/public/request/requestInfoDetail.js"
	...
	import {hasResponse,sign,getUserInfo} from "@/api/sign/sign.js"
	export default{
		onShow(options) {
			if(this.channel == '1'){
				this.validateStatus();
		...
		onShareAppMessage(res) {
			return new Promise((resolve, reject) => {
				...
				sign(this.request.id).then(response => {
		...
		async onLoad(options){
			console.log("requestInfoDetail,onLoad:",options)
		...
		methods:{
			...
			goProcessDetail(id) {
				//测试数据没有写id，用title代替
				uni.navigateTo({
					url: `/pages/public/process/processView_request?id=${id}`
				})
			},
			imageLoad(event){
				const msgIndex = event.target.dataset.index;
				this.message[msgIndex].loaded = true;
			},
			inputMessage(event){
			...
			goResponseDetail(id){
				uni.navigateTo({
					url: `/pages/public/response/responseViewDetail?id=${id}`
				}) 
			},
			doResponse(id){
				if(!this.haslogin){
					...
					hasResponse(this.request.id).then(response => {
						if(response.data != "0"){
							this.$modal.msgError("已有回复。。");
							return;
						}else{
							uni.navigateTo({
								url: `/pages/public/response/addResponse?id=${id}`
							})
						}
					});
				}
