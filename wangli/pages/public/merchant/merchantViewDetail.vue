<template>
	<view class="container">
			...
			<view class="top-title-container" :style="{backgroundColor:merchant.backgroundColor,background:'url('+merchant.background+')'}">
				...
				<view class="top-title-info">
					<view class="top-title-list" :style="{backgroundColor:merchant.backgroundColor}">
						...
						<view class="top-title-row b-b">
							<text class="top-title-tit" :style="{color:merchant.tagColor}">创建者：</text>
							<text class="top-title-text" :style="{color:merchant.contentColor}">{{merchant.createUserName}}</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="c-list" :style="{backgroundColor:merchant.backgroundColor,background:'url('+merchant.background+')'}">
			...
			<view class="c-row b-b">
				<text :style="{color:merchant.tagColor}">材料(证明你是你，能让用户相信你的材料)</text>
			</view>
			<view class="c-row b-b">
				<view class="con">
			...
			<view class="seckill-section m-t" v-if="product.length > 0">
				<scroll-view class="floor-list" scroll-x>
					<view class="scoll-wrapper">
						<view 
							v-for="(productItem, productIndex) in product" :key="productIndex"
							class="floor-item"
						>
							<image :src="productItem.tagUrl" mode="aspectFill" @click="viewProduct(productItem.id)"></image>
							<text class="title clamp">{{productItem.name}}</text>
						</view>
					</view>
				</scroll-view>
			</view>
			<view class="seckill-section m-t" v-if="product.length == 0">
				<text class="title clamp">无</text>
			</view>
...
<script>
	import {getMerchantInfo,getProductInfo} from "@/api/public/merchant/merchantView.js"
	export default{
		data() {
			return {
				merchant: {
