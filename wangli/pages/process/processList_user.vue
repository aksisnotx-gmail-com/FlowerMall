					...
					>
						...
						<view class="item-right c-list">
							...
							<view class="c-row b-b">
								<view class="tit">所属区域：</view>
								<text class="con">{{item.areaText}}</text>
							</view>
							<view class="c-row b-b">
								<view class="tit">类型：</view>
								<text class="con">{{item.typeText}}</text>
							</view>
							<view class="c-row b-b">
								<view class="tit">创建人：</view>
...
<script>
	...
	import {getProcess,getProcessByRequestId,initProcessSign} from "@/api/process/processList.js"
		...
		methods: {
			...
			loadData(requestId){
				...
				if(requestId != undefined){
					getProcessByRequestId(requestId).then(response => {
			...
			toProcessDetail(id,status){
				uni.navigateTo({
					url: `/pages/process/processView_user?id=${id}`
				})
			},
			//监听image加载完成
