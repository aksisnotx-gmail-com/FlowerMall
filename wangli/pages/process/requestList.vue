					...
					>
						<view class="image-wrapper">
							<image :src="item.tagUrl" 
								...
								mode="aspectFill" 
								lazy-load 
								@click="toRequestDetail(item.id,item.status)"
						...
						<view class="item-right c-list">
							...
							<view class="c-row b-b">
								<view class="tit" v-if="item.status == '1' ">发布：</view>
								<view class="tit" v-if="item.status != '1' ">自动发布：</view>
								<view class="con">
...
<script>
	...
	import {getRequestInfo} from "@/api/request/requestList.js"
	export default {
		data() {
			return {
				total: 0, //总价格
		...
		methods: {
			...
			async loadData(){
				...
				let active = prePage.$vm.active;
				let requestList = prePage.$vm.detail.formData[active].requestList;
				for(let request of requestList){
					this.requestList.push(request.id);
				}
				this.cartList = [];
				await getRequestInfo().then(response => {
					for(let data of response.data){
						if(this.requestList.includes(data.id)){
			...
			requestConfirm(){
				...
				for(let data of this.cartList){
					if(data.checked && !this.requestList.includes(data.id)){
						requestList.push(data)
					}
				}
				if(this.count > 3){
					this.msgType = "error"
					this.messageText = `一个流程最多关联三个任务`
					this.$refs.message.open()
					return;
				}else{
					for(let data of requestList){
						prePage.$vm.detail.formData[active].requestList.push(data)
					}
				}
			...
			check(index){
				...
				let prePage = pages[pages.length - 2];
				let active = prePage.$vm.active;
				let requestLength = prePage.$vm.detail.formData[active].requestList.length;
				...
				if(requestLength + countTmp > 3){
					this.msgType = "error"
					this.messageText = `最多选择三个任务`
					this.$refs.message.open()
					return;
				}else{
					this.count = countTmp;
